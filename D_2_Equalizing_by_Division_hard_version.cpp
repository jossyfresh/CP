...
#define allk(x,n) x.begin(), x.begin() + n
#define fi first
#define se second
#define pb push_back
#define mp make_pair
...
void solve() {
    ...
    cin >> n >> k;
    vector<int> arr;
    vector<vector<int>>possible(2 * 1e5 + 1 );
    si pos;
    IL(arr,n);
    int ans = INF;
    ...
    forit(it, pos){
        int val = *it;
        vector<int>count = possible[val];
        if (count.size() >= k){
            sort(all(count));
            ans = min(ans, accumulate(allk(count,k), 0));
        }
    }
    cout << ans << endl;
}
