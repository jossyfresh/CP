...
void makearray(int l,int num,vector<int> &ans,vector<int> &arr, int &count){
    bool flag = false;
...
void makearray2(int l,int num,vector<int> &ans,vector<int> &arr, int &count){
    bool flag = false;
    for (int i = l;i < ans.size();i++){
        if (arr[i] == num){
            ans[i] = 3;
...
void solve() {
    ...
    int count = 0;
    for (int i = 0;i < n;i++){
        makearray(i+1,arr[i],ans,arr,count);
        makearray2(i+1,arr[i],ans,arr,count);
    }
    cout << count << " ";
        display(ans);
    cout << endl;
